...
In the event of a brake cable disconnection, the motor can be shut off using the panic button, by closing the app, or by disconnecting the battery.

Software programming is inherently prone to errors. Consequently, a prudent approach to 'life-critical' software necessitates thorough bench testing. This should be conducted with the wheels off the ground or the chain removed to verify speed accuracy and ensure user comfort with the system's operation.

It is important to note that observing the crank spinning alone does not accurately represent real-world acceleration and speed. Therefore, initial testing should be performed at low speeds and with gradual acceleration.
...
Vesc can only have 1 user QML app running. 
If the ADC app or any other buit in app is running that uses any of the three main motor controls: SetRPM, SetDuty or SetCurrent - will *FIGHT*
...
If you do use Vesc - please do contribute and or give a small donation

https://vesc-project.com/donations

Unoffical Discord server - please note this is not a support forum for a paid commercial product, for that kind of support talk to the manufacturer of your vesc device.
...
```
// Shader header used by every shader
...
There is one potential big issue with a single gear setup - if you have vertical dropouts (the slot on the bike that the wheel axel fits into) it's impossible to get the chain tight and it will pop off very quickly. That's because a chain link on a normal bike are two links in length (so you can only construct and even number of links on a chain) and the dropout doesn't have enough room to pull the chain tight. A BMX style half-link chain wont work on normal bike gears - different teeth size e.t.c. - so you could  rig a single link together with some pliers e.t.c. but it probably would be a right old mess. The only realistic solutions are to either - fix a derailleur into a single gear position, or get a dual pulley chain tensioner. Dual-Pulley Chain tensioners are typically really short reaching so on the normal derailuer mounting point - so the maxium rear gear size is around 32t - which is fine if u want speed and slight hills - but bad if you want low a gearing hill monster. You have to mount the tensionr on the rear bike frame arm somehow (thermo plastic) if you want monster 52T's.
